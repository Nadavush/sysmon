...
def make_disk_table(disk_readings):
    ...
    for part in partitions_data_list:
        partition_mountpoint = part["partition-mountpoint"]
...
def make_network_table(first_time_flag, network_readings):
    ...
    bytes_sent = network_readings["sent"]
    bytes_recv = network_readings["received"]
    if first_time_flag:
        upload_speed = "TBD"
        download_speed = "TBD"
    else:
        upload_speed = network_readings["upload-speed"]
        download_speed = network_readings["download-speed"]
